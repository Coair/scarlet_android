...
public class SMS4 {
	...
	private static final int OP_DECRYPT = 0;
	...
	private static byte[] stringToHexPoint(String str) {
		byte[] bytes = new byte[str.length() / 2];
		int inputIndex = 0;
		for (int index = 0; index < str.length(); index += 2) {
			String tmp = str.substring(index, index + 2);
			byte temp1 = stringToBCDHex(tmp.substring(0, 1));
			byte temp2 = stringToBCDHex(tmp.substring(1, 2));
			inputIndex = index / 2;
			bytes[inputIndex] = (byte) ((temp1 << 4) + temp2);
		}
		return bytes;
	}

	private static byte stringToBCDHex(String strIn) {
		byte result = 0x00;
		if ("0".equals(strIn)) {
			result = 0x00;
		} else if ("1".equals(strIn)) {
			result = 0x01;
		} else if ("2".equals(strIn)) {
			result = 0x02;
		} else if ("3".equals(strIn)) {
			result = 0x03;
		} else if ("4".equals(strIn)) {
			result = 0x04;
		} else if ("5".equals(strIn)) {
			result = 0x05;
		} else if ("6".equals(strIn)) {
			result = 0x06;
		} else if ("7".equals(strIn)) {
			result = 0x07;
		} else if ("8".equals(strIn)) {
			result = 0x08;
		} else if ("9".equals(strIn)) {
			result = 0x09;
		} else if ("A".equals(strIn.toUpperCase())) {
			result = 0x0A;
		} else if ("B".equals(strIn.toUpperCase())) {
			result = 0x0B;
		} else if ("C".equals(strIn.toUpperCase())) {
			result = 0x0C;
		} else if ("D".equals(strIn.toUpperCase())) {
			result = 0x0D;
		} else if ("E".equals(strIn.toUpperCase())) {
			result = 0x0E;
		} else if ("F".equals(strIn.toUpperCase())) {
			result = 0x0F;
		}
		return result;
	}

	public static String decrypt(String str,String charset) {
		String rtn = null;
		byte[] key = { 0x44, 0x44, 0x44, 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44,
				(byte) 0x44, (byte) 0x44, (byte) 0x44, 0x44, 0x44, 0x44, 0x44 };
		int strLength = str.length();
		int inLen = 16 * (strLength / 32 == 0 ? 1 : strLength / 32), DECRYPT = 0;
		byte[] out = new byte[inLen];
		byte[] mi = stringToHexPoint(str);
		// 解密 128bit
		sms4(mi, inLen, key, out, DECRYPT);
		try {
			rtn = new String(out, charset).trim();
	...
	public static String encrypt(String str,String charset) throws IOException {
		byte[] key = { 0x44, 0x44, 0x44, 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44, (byte) 0x44,
				(byte) 0x44, (byte) 0x44, (byte) 0x44, 0x44, 0x44, 0x44, 0x44 };
		byte[] mi = str.getBytes(charset);
		int length = mi.length;
		int ENCRYPT = 1;
		int inLen = (int) (16 * (Math.ceil(length / 16.0)));
		StringBuffer sb = new StringBuffer();
		byte[] out = new byte[inLen];
		sms4(mi, inLen, key, out, ENCRYPT);
		for (int i = 0; i < inLen; i++) {
			sb.append(toFullHexString(out[i] & 0xff));
		}
		return sb.toString();
	}
	 ...
	 public static void main(String[] args) throws IOException {
	    	String a = "\"你好\"";
	    	String strEncrypt=SMS4.encrypt(a,"UTF-8");
	        System.out.println(strEncrypt);
	        String strDecrypt=SMS4.decrypt(strEncrypt,"UTF-8");
